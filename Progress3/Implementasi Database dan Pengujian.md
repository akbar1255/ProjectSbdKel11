...
## 2. CONSTRAINT YANG DITERAPKAN

| Tabel               | Constraint       | Keterangan                                               |
|---------------------|------------------|----------------------------------------------------------|
| `dosen`             | PRIMARY KEY      | `nidn`                                                   |
| `dosen`             | UNIQUE           | `email`                                                  |
| `dosen`             | NOT NULL         | `nama`, `program_studi`, `fakultas`, `email`             |
| `skema`             | PRIMARY KEY      | `id_skema` (AUTO_INCREMENT)                              |
| `skema`             | NOT NULL / ENUM  | `jenis` hanya 'penelitian' atau 'pengabdian'             |
| `proposal`          | PRIMARY KEY      | `id_proposal`                                            |
| `proposal`          | FOREIGN KEY      | `id_skema` → `skema(id_skema)`                           |
| `proposal`          | ENUM / DEFAULT   | `status` default 'diajukan'                              |
| `anggota_proposal`  | FOREIGN KEY (×2) | ke `proposal` dan `dosen`                                |
| `rab`               | FOREIGN KEY      | `id_proposal` → `proposal(id_proposal)`                  |
| `review`            | FOREIGN KEY (×2) | ke `proposal` dan `reviewer`                             |
| `review`            | CHECK            | `skor BETWEEN 0 AND 100`                                 |
| `riwayat_status`    | FOREIGN KEY      | `id_proposal` → `proposal(id_proposal)`                  |
| `kontrak`           | UNIQUE           | `id_proposal` (satu proposal satu kontrak), `no_kontrak` |
| `kontrak`           | FOREIGN KEY      | `id_proposal` → `proposal(id_proposal)`                  |
| `termin_pencairan`  | FOREIGN KEY      | `id_kontrak` → `kontrak(id_kontrak)`                     |
| `laporan`           | FOREIGN KEY      | `id_proposal` → `proposal(id_proposal)`                  |
| `luaran`            | FOREIGN KEY      | `id_proposal` → `proposal(id_proposal)`                  |

---

## 4.3 SQL DML (Data Uji)
...
| No | Skenario                                              | Tabel yang Terlibat                          | Jenis Query          |
|----|-------------------------------------------------------|----------------------------------------------|----------------------|
| 1  | Menampilkan seluruh data dosen                        | `dosen`                                      | SELECT               |
| 2  | Menampilkan proposal beserta nama skema               | `proposal`, `skema`                          | SELECT + JOIN        |
| 3  | Menampilkan dosen beserta proposal yang diajukan      | `dosen`, `anggota_proposal`, `proposal`      | SELECT + JOIN        |
| 4  | Menampilkan hasil review beserta nama reviewer        | `review`, `reviewer`, `proposal`             | SELECT + JOIN        |
| 5  | Menampilkan total anggaran RAB per proposal           | `rab`, `proposal`                            | SELECT + GROUP BY    |
| 6  | Menampilkan proposal yang berstatus diterima          | `proposal`, `kontrak`                        | SELECT + WHERE       |
| 7  | Menampilkan rekap jumlah proposal per skema per tahun | `proposal`, `skema`                          | SELECT + GROUP BY    |
| 8  | Menampilkan status pencairan dana per kontrak         | `kontrak`, `termin_pencairan`, `proposal`    | SELECT + JOIN        |
| 9  | Memperbarui status proposal menjadi revisi            | `proposal`, `riwayat_status`                 | UPDATE + INSERT      |
| 10 | Menampilkan dosen yang belum pernah mengajukan proposal | `dosen`, `anggota_proposal`                | SELECT + LEFT JOIN   |
...
-- ============================================================
-- QUERY 3: Menampilkan dosen beserta proposal yang diajukan
-- (termasuk peran sebagai ketua atau anggota)
-- ============================================================
SELECT
  d.nidn,
  d.nama,
...
-- ============================================================
-- QUERY 4: Menampilkan hasil review beserta nama reviewer
-- dan judul proposal
-- ============================================================
SELECT
  r.id_review,
...
-- ============================================================
-- QUERY 6: Menampilkan proposal yang berstatus diterima
-- beserta info kontrak
-- ============================================================
SELECT
  p.id_proposal,
  p.judul,
...
-- ============================================================
-- QUERY 9: Update status proposal menjadi 'revisi' dan
-- catat perubahan ke riwayat_status
-- ============================================================
UPDATE proposal
SET status = 'revisi'
WHERE id_proposal = 3;
...
-- ============================================================
-- QUERY 10: Menampilkan dosen yang BELUM pernah mengajukan
-- proposal (menggunakan LEFT JOIN)
-- ============================================================
SELECT
  d.nidn,
  d.nama,
...
-- ============================================================
-- QUERY BONUS: Rekap luaran per jenis untuk seluruh proposal
-- yang diterima
-- ============================================================
SELECT
  lu.jenis_luaran,
  COUNT(lu.id_luaran) AS jumlah_luaran,
  GROUP_CONCAT(p.judul SEPARATOR ' | ') AS dari_proposal
FROM luaran lu
JOIN proposal p ON lu.id_proposal = p.id_proposal
WHERE p.status = 'diterima'
GROUP BY lu.jenis_luaran
ORDER BY jumlah_luaran DESC;
``
